...
int recois_envoie_message(int socketfd) {
   ...
   */
  printf ("Message serveur recu: %s\n", data);
  char code[10];
  memset(code, 0, sizeof(code));
  /*char op = '*';
  int num1 = 0;
  int num2 = 0;*/
  //sscanf(data, "%s: %c %i %i", code, op, num1, num2);
  //sscanf(data, "\"%s\": \"%c\": \"%d\": \"%d\":", code, &op, &num1, &num2);
  //sscanf(data, "%s:",code);
  ...
  /*if (strcmp(code, "calcule") == 0) {
    float calcul;
    calcul = recois_numeros_calcule(data);
...
default: printf("Rentrer un operateur valide\n");
